...
+function ($) {
    ...
    function closeExternal (event) {
        ...
        if (isOverMinWidth.apply(this) || $(event.target).parents('.' + this.options.classWrapper).size() > 0 || $target.parents('.' + this.options.classWrapper).size() > 0 || $target.hasClass('sidebar-swipe')) {
            return;
        }
